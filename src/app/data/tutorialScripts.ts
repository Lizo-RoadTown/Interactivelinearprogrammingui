...
const TOY_FACTORY_PHASE4: Question[] = [
  ...
  // Column indices in the initial tableau: 0=x1, 1=x2, 2=s1, 3=s2. Rows: 0=C1, 1=C2.
  {
    ...
    id: 'toy-p1-entering',
    phase: 4,
    prompt: 'PRINCIPLE: you\'re about to pick which variable ENTERS the basis. That\'s one action described two ways:\n\n• In the TABLEAU: the column with the most negative z-row value grows z the fastest per unit.\n• On the GRAPH: the entering variable picks which direction you SLIDE from the current corner toward a neighboring corner.\n\nWhich variable should enter the basis?',
    pick: 'entering-col',
    correctIndex: 1, // x2 column
    hint: 'The z-row is (−15, −20, 0, 0). −20 is the most negative, so x₂ enters the basis. Picking x₂ = sliding UP the x₂ axis from the origin.',
...
const TOY_FACTORY_PHASE6: Question[] = [
  ...
  // principle-first prompts (principle 3), clues not answers (principle 11).
  {
    ...
    phase: 6,
    prompt: 'PRINCIPLE: every corner of the feasible region IS a basis (a choice of which variables are > 0). Dots mark each corner of your feasible polygon. Which corner is the optimum — where both constraints are tight and z* = 450?',
    targetX: 10,
    targetY: 15,
    tolerance: 0.5,
  ...
  // ── Piece 2: build B by clicking basic columns in A ─────────────────────
  {
    ...
    id: 'toy-s-b-col-0',
    phase: 6,
    prompt: 'PRINCIPLE: B is just the columns of A that correspond to the basic variables. Your basis is { x₁, x₂ }. Slot 1 of B takes the x₁ column from the original constraints. Which column of A holds x₁\'s constraint coefficients?',
    targetColumn: 0,
  ...
  },
  {
    ...
    id: 'toy-s-b-col-1',
    phase: 6,
    prompt: 'Slot 2 of B takes the x₂ column. Which column of A holds x₂\'s constraint coefficients?',
    targetColumn: 1,
